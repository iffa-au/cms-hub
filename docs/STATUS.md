...
returns `countryName` (backend). The review queue and All submissions show a
Country column, and the review queue gained the same Filters dialog as All
submissions minus status — country, language, year, categories, genres.
"Download all as PDF" follows those filters as well as the search text. The
dialog now lives in `components/submissions/advanced-filters.tsx`, shared by
both pages. Backend must deploy first or the column shows "—".

Not clicked through in a browser by the agent — needs an authenticated session.

**Film enquiry PDF.** `/admin/film-enquiry/[id]` has a Download PDF button
beside Delete enquiry, built client-side from the loaded enquiry by
`lib/film-enquiry-pdf.ts` (same layout as the submission PDF). Rendered against
real enquiries, including an 18k-character synopsis (5 pages). 274 of 289
enquiries have no contact name or email (old imports store `null` for most
fields); their PDFs are mostly "—", as their pages are.

Also: native `<option>` lists are now coloured explicitly in `globals.css`
(Edge drew them white), and `layout.tsx` sets `darkreader-lock` +
`color-scheme: dark` so Dark Reader stops recolouring the dark-only CMS.
...
## Most archive films have no resolvable country

Found 2026-09-23 while adding a Country column to the review queue and
submissions list (`countryName`, joined from `countryId` in
`adminListSubmissions`). Of 361 approved films, **251 resolve to no country**:

- **218** carry a `countryId` that matches nothing in `countries` (203 docs).
  40 distinct ids, one used 80 times. They are not real ObjectIds — the
  embedded timestamps decode to 1978, 2061 etc. — so they were generated by the
  2022–2025 bulk import, whose countries were never imported under those ids.
- **33** have no `countryId` at all (2023: 14, 2025: 19).

All 2026 films resolve. The CMS shows "—" for the rest, in the list and on the
detail page alike. Recovering them needs the original import's id→name
mapping, or a manual re-pick per film; the ids alone say nothing.
