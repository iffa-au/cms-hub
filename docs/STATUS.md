...
**Credit roles — the "Other Crew" vocabulary.** Two stacked PRs: #30 (backend)
and #31 (client, based on #30 so it cannot reach production first). New
`CreditRole` model and `/credit-roles` CRUD (public GET, admin writes).
...
projection strips biography, instagram, email, phone and notes — and the
editor saves all four groups wholesale, so any edit there overwrote those five
fields with empty strings). The edit page's error fallback read the same route
and now refuses to edit rather than loading a reduced copy.

Credited role is a dropdown now: fixed sets for directors, producers and cast;
`/credit-roles` for Other Crew. A stored role that no list contains stays
selected and is labelled "as submitted" — most existing crew was typed
freehand on the public form, so `Director/Writer` and `DOP` are common and
must survive a save untouched.

**Neither screen has been opened in a browser.** Both need an authenticated CMS
session. Worth confirming the crew thumbnails on `/submissions/[id]/view`, the
four dropdowns, and the new Metadata → Crew Roles panel.
